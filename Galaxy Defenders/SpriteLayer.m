...
-(void) update:(ccTime)deltaTime
{
    ...
    for (CCSprite *en in enemies)
    {
        if ((rand() % 100 + 1) == 50)
        {
            ...
            [self from:en.position to:ccp(en.position.x, 0) over:1.0f with:2];
        }
    }
    ...
    {
        [[SimpleAudioEngine sharedEngine]playEffect:@"PewPew.wav"];
        [self from:player.position to:ccp(player.position.x, 768) over:1.0f with:1];
    }
    if (health <= 0)
    {
...
-(void)from:(CGPoint)start to:(CGPoint)end over:(float)duration with:(int)type
{
    switch (type) {
        case 1:
        {
